<script lang="ts">
	interface Props {
		topic: string;
		label: string;
		currentSubLevel: number;
		maxSubLevel?: number;
	}

	let { topic, label, currentSubLevel, maxSubLevel = 10 }: Props = $props();

	const dots = $derived(
		Array.from({ length: maxSubLevel }, (_, i) => ({
			level: i + 1,
			status:
				i + 1 < currentSubLevel
					? 'completed'
					: i + 1 === currentSubLevel
						? 'current'
						: ('locked' as 'completed' | 'current' | 'locked')
		}))
	);

	const topicIcons: Record<string, string> = {
		addition: '+',
		subtraction: '-',
		multiplication: '\u00D7',
		counting: '#',
		ordering: '<>'
	};

	const topicColors: Record<string, string> = {
		addition: 'bg-success',
		subtraction: 'bg-info',
		multiplication: 'bg-xp-gold',
		counting: 'bg-gem-purple',
		ordering: 'bg-warning'
	};
</script>

<div class="rounded-[--radius-lg] bg-surface p-4 shadow-sm">
	<div class="mb-3 flex items-center gap-2">
		<span
			class="flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold text-white {topicColors[
				topic
			] ?? 'bg-primary'}"
		>
			{topicIcons[topic] ?? '?'}
		</span>
		<span class="font-semibold text-text">{label}</span>
		<span class="ml-auto text-xs text-text-muted">
			{currentSubLevel} / {maxSubLevel}
		</span>
	</div>

	<div class="flex gap-1.5">
		{#each dots as dot (dot.level)}
			<div
				class="h-3 flex-1 rounded-full transition-all duration-300
					{dot.status === 'completed'
					? (topicColors[topic] ?? 'bg-primary')
					: dot.status === 'current'
						? (topicColors[topic] ?? 'bg-primary') + ' animate-pulse'
						: 'bg-gray-200'}"
			></div>
		{/each}
	</div>
</div>
